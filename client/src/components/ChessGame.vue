...
export default {
  ...
  methods: {
    onClickTile(value) {
      if (this.board[value] == "" && this.clickedTiles.length == 0) return;
      ...
      if (this.clickedTiles.length == 2) {
        if (
          this.availableMoves.filter(
            (m) =>
              m.from == this.clickedTiles[0] && m.to == this.clickedTiles[1]
          ).length > 0
        ) {
          if (
            (this.clickedTiles[0] == "" && this.clickedTiles[1] == "") ||
            this.board[value][0] == this.board[this.clickedTiles[0]][0]
          ) {
  ...
  mounted() {
    ...
    this.socket.on("chessboardMove", (move) => {
      ...
      let audio;
      if (this.board[move.to] != "") audio = new Audio("sounds/capture.mp3");
      else audio = new Audio("sounds/move.mp3");
